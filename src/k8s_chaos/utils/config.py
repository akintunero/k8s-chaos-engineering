...
class Config:
    ...
    def from_file(cls, config_path: Optional[str] = None) -> "Config":
        ...
        # Resolve relative paths against data root (PyPI wheel + git checkout)
        from k8s_chaos._paths import data_root

        root = data_root()
        for key, sub in (
            ("experiments_dir", "experiments"),
            ("manifests_dir", "examples/quickstart"),
        ):
            value = config_dict.get(key, sub)
            path = Path(value)
            if not path.is_absolute():
                config_dict[key] = str(root / value)
