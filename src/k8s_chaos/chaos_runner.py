...
def list_experiments():
    ...
    for file in experiments_dir.glob("*.yaml"):
        stem = file.stem
        if is_experiment_file(stem):
            experiments.append(stem)

    profile = get_profile()
    return [name for name in experiments if name in profile.allowed_experiments]
...
def run_experiment(experiment_name: str, namespace: str = None):
    ...
    try:
        ...
        assert_experiment_allowed(experiment_name)
    ...
    if os.getenv("SKIP_PREFLIGHT", "").lower() not in ("1", "true", "yes"):
